...
internal sealed class ClientApplicationContext : ApplicationContext
{
    ...
    private async Task InitializeAsync(ToolStripMenuItem startItem, ToolStripMenuItem restoreItem)
    {
        ...
        {
            MessageBox.Show(error.Message, $"{_options.Product.DisplayName} 客户端无法启动", MessageBoxButtons.OK, MessageBoxIcon.Error);
            ExitClient();
        }
    }
    ...
    private async Task RunActionAsync(string action, ToolStripMenuItem source)
    {
        ...
        {
            MessageBox.Show(error.Message, _options.Product.DisplayName, MessageBoxButtons.OK, MessageBoxIcon.Warning);
        }
        finally
        {
            source.Enabled = true;
