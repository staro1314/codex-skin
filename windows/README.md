...
Codex Skin 通过本机回环 CDP 给官方 Codex Windows 桌面应用加载外部主题。它保留原生侧栏、项目选择、任务内容和输入框，不修改 WindowsApps、`app.asar` 或应用签名。
...
`CodexSkin-Setup-vX.Y.Z.exe`，按 [`docs/install-windows.md`](../docs/install-windows.md) 的图形
...
正式 Release Setup 只部署客户端、控制中心和受管运行时，不调用浏览器测试安装流程。首次安装、更新和
重新安装都不要求 Codex 打开，也不修改 Codex 的 `config.toml`；更新前只会停止 Codex Skin 自身的客户端、
托盘和 Node 服务，再原子替换受管运行时。卸载同样只停止 Codex Skin 自身进程，并恢复已保存的配置；不会
强制关闭 Codex。若 Codex 当时仍在运行，当前窗口中已经加载的皮肤会在 Codex 下次重启后消失。主题和图片
仍保存在 `%LOCALAPPDATA%\CodexDreamSkin`，不会因更新或卸载安装程序而删除。

普通使用不需要管理员权限，也不需要接管 WindowsApps 目录。

## 高级：从源码安装

普通用户无需阅读本节。在 PowerShell 中进入仓库的 `windows` 目录，然后运行。该命令是浏览器测试/源码安装流程，会立即写入 `config.toml`，因此仍要求 Codex 退出；正式 Setup 不调用它：
...
- `Codex Skin`：打开原生控制中心客户端。
- `Codex Skin - Tray`：以后台模式启动客户端并驻留系统托盘。
- `Codex Skin - Restore`：恢复官方外观并关闭已保存的 CDP 会话。
...
源码安装流程更新前先退出 Codex Skin 托盘并关闭 Codex。正式 Setup 会先处理已登记的旧安装，再原子
替换受管运行时并重建快捷方式；当前主题、已保存主题和导入图片不会被删除。

## 启动与验证

推荐从 `Codex Skin` 快捷方式启动客户端。关闭窗口只会隐藏到托盘；从托盘退出客户端才会停止其内部控制中心服务。客户端的“启动 / 重新应用 Codex”仍会调用经过验证的 PowerShell 启动流程，并在 Codex 已经运行时先询问是否重启。
...
打开 `Codex Skin` 客户端后可以（后台启动时也可从托盘双击打开）：
...
如需同时删除 Codex Skin 创建的快捷方式，再增加 `-Uninstall`：
...
| 用途 | 路径 |
|------|------|
| Codex Skin 状态根目录 | `%LOCALAPPDATA%\CodexDreamSkin` |
...
通过 `Codex Skin` 快捷方式启动 Codex，再运行验证脚本。普通 Codex 启动方式不会打开 Codex Skin 所需的调试会话。
...
提交问题时请从仓库的 [Issue 提交页](https://github.com/staro1314/codex-skin/issues/new/choose) 选择 Bug 模板，附上系统版本、Codex 来源、复现步骤和相关日志片段。请删除密钥、`auth.json`、中转 token 和私人对话内容。
