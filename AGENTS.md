...
- 以后排查或修改任何窗口、浮框、面板或输入区域的透明度时，先阅读 `code-help/TRANSPARENCY_ADJUSTMENT_RECORD.md`，优先复用其中记录的触发器、精确选择器、运行时 marker、透明度/模糊层级和非目标边界。
- 记录文档不是替代验证的猜测依据：动手前必须用当前 `runtime/` 源码、`tools/selectors.json` 和实际计算样式核对是否发生漂移；修改后必须按文档中的验收标准验证，并把新的事实、文件位置和验证结果回写文档。
- 不得通过关闭目标浮框、只截取关闭状态或扩大 CSS 选择器范围来制造“已修复”的证据；输入框透明规则与各类浮框规则必须保持隔离。
...
- 产品显示名、Studio 名称、安装包文件名和发布者只修改仓库根目录 `PRODUCT.json`，然后运行 `node tools/sync-runtime-assets.mjs` 生成双端产品副本；不要手工修改 `windows/assets/`、`macos/assets/` 或 `runtime/` 下的生成配置。`CodexDreamSkin` 等技术标识、状态目录、进程名和协议标识属于兼容性边界，不随显示名重命名。
