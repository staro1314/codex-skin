...
1. 在 GitHub 的 [Releases](https://github.com/staro1314/codex-skin/releases) 下载最新的
   `CodexSkin-vX.Y.Z.dmg`。`SHA256SUMS.txt` 是可选的完整性校验文件。
...
手动方式：从菜单选择“打开主题文件夹”，把已经解压且直接包含 `theme.json`、`theme.css`、背景图和可选视频的完整目录放入
...
2. 退出 Codex Skin 菜单栏应用，并按提示关闭 Codex。
...
确认旧的 Codex Skin 已退出，再启动 Applications 中的新版本。若 Codex 正在运行，允许应用按
