...
这是当前 Windows 客户端实际运行中的皮肤效果：背景、玻璃表面、内容可读性和状态装饰直接作用于 Codex 原生界面，同时保留原生菜单、任务内容和输入区域的正常交互。左侧项目区域已做脱敏处理：

![当前客户端实际皮肤效果（项目区域已脱敏）](./docs/images/current-client-skin-redacted.jpg)

这不是浏览器控制中心或模拟预览，而是客户端真实渲染结果。主题不是整窗截图覆盖，而是在官方 Codex 界面上叠加受控的背景与视觉层；你可以继续使用原生控件，也可以在控制中心中实时调整并保存自己的主题。
...
- 自动化验证：覆盖主题包、渲染注入、选择器、窗口可见性、恢复、安装器静态内容和发布资产。
...
3. 完善真实 Codex 环境中的页面状态采样、事件桥和性能验证。
