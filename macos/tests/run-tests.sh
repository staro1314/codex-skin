...
CANONICAL_VERSION="$(/usr/bin/tr -d '[:space:]' < "$ROOT/../VERSION")"
...
"$NODE" -e '
  ...
  if (value.currentVersion !== `v${expectedVersion}` || value.latestVersion !== "v9.8.7") process.exit(1);
...
/usr/bin/env -u HOME /bin/bash -c '. "$1/scripts/common-macos.sh"; [ -n "$HOME" ] && [ "$SKIN_VERSION" = "$2" ]' _ "$ROOT" "$CANONICAL_VERSION"
