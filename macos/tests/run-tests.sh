...
"$NODE" -e '
  ...
  if (value.currentVersion !== "v1.0.1" || value.latestVersion !== "v9.8.7") process.exit(1);
...
/usr/bin/env -u HOME /bin/bash -c '. "$1/scripts/common-macos.sh"; [ -n "$HOME" ] && [ "$SKIN_VERSION" = "1.0.1" ]' _ "$ROOT"
