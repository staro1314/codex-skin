...
if [ "$CREATE_LAUNCHERS" = "true" ]; then
  ...
  screenshot="$(shell_quote "$HOME/Desktop/Codex-Skin Verification.png")"
  write_launcher "$HOME/Desktop/Codex-Skin.command" "exec $start_script --port $PORT --prompt-restart"
  write_launcher "$HOME/Desktop/Codex-Skin - Customize.command" "exec $customize_script"
  write_launcher "$HOME/Desktop/Codex-Skin - Verify.command" "$verify_script --screenshot $screenshot && /usr/bin/open $screenshot"
  write_launcher "$HOME/Desktop/Codex-Skin - Restore.command" "exec $restore_script --restore-base-theme --restart-codex"
fi

printf 'Codex-Skin %s installed at %s for Codex %s using its signed Node.js %s.\n' \
