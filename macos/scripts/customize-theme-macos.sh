...
if [ "$RESET_DEMO" = "true" ]; then
  ...
  if [ -z "$THEME_NAME" ]; then
    THEME_NAME="$(/usr/bin/osascript -e 'text returned of (display dialog "给这套主题起个名字" default answer "我的 Codex-Skin" buttons {"取消", "继续"} default button "继续")')" \
...
printf 'Codex-Skin theme is ready.\n'
